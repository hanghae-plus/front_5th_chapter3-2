...
const setupMockHandlers = (initialEvents: Event[] = []) => {
...
describe('반복 유형 선택 통합 테스트', () => {
  ...
  // 매월 반복 - 31일 처리 테스트
  it('31일에 매월 반복 일정을 설정하면 30일까지만 있는 달을 적절히 처리해야 한다', async () => {
    setupMockHandlers();
    ...
    // 일정 기본 정보 입력
    await user.type(screen.getByLabelText('제목'), '월말 보고서');
    ...
    await user.type(screen.getByLabelText('설명'), '월말 보고서 제출');
    ...
    // 반복 종료일 설정
    await user.type(screen.getByLabelText('반복 종료일'), '2024-07-31');
    ...
    // 저장 후 일정 목록에 추가되었는지 확인
    const eventList = within(screen.getByTestId('event-list'));
    expect(eventList.getByText('월말 보고서')).toBeInTheDocument();
  });

  // 매년 반복 - 윤년 2월 29일 처리 테스트
  it('윤년 2월 29일에 매년 반복 일정을 설정하면 비윤년을 적절히 처리해야 한다', async () => {
    setupMockHandlers();
    ...
    // 일정 기본 정보 입력
    await user.type(screen.getByLabelText('제목'), '윤년 기념 일정');
    await user.type(screen.getByLabelText('날짜'), '2024-02-29'); // 윤년 2월 29일
    await user.type(screen.getByLabelText('시작 시간'), '12:00');
    await user.type(screen.getByLabelText('종료 시간'), '13:00');
    await user.type(screen.getByLabelText('설명'), '윤년 기념 행사');
    await user.type(screen.getByLabelText('위치'), '강당');
    ...
    // 반복 종료일 설정
    await user.type(screen.getByLabelText('반복 종료일'), '2026-03-01');
    ...
    // 저장 후 일정 목록에 추가되었는지 확인
    const eventList = within(screen.getByTestId('event-list'));
    expect(eventList.getByText('윤년 기념 일정')).toBeInTheDocument();
  });
});
