...
test('매일 반복 일정을 생성하면 여러 날짜에 표시된다', async ({ page }) => {
  ...
  await page.getByLabel('날짜').fill('2025-07-07');
  ...
  // 반복 설정
  await page.getByLabel('반복 일정').check(); // checkbox ON
  ...
  await page.getByLabel('반복 종료일').fill('2025-07-10');
  ...
  await expect(eventList).toContainText('2025-07-07');
  await expect(eventList).toContainText('2025-07-08');
  await expect(eventList).toContainText('2025-07-09');
  await expect(eventList).toContainText('2025-07-10');
});
