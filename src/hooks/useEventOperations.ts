...
export const useEventOperations = (editing: boolean, onSave?: () => void) => {
  ...
  const createRepeatEvent = async (eventForms: EventForm[]) => {
    ...
    const expanded: EventForm[] = eventForms.flatMap((form) => {
      switch (form.repeat.type as RepeatType) {
        case "daily":
          return generateDailyDates(form).map((date) => ({ ...form, date }));
        case "weekly":
          return generateWeeklyDates(form).map((date) => ({ ...form, date }));
        case "monthly":
          return generateMonthlyDates(form).map((date) => ({ ...form, date }));
        case "yearly":
          return generateYearlyDates(form).map((date) => ({ ...form, date }));
        default:
          return [form];
      }
    });
  ...
  const updateRepeatEvents = async (eventForms: Event[]) => {
    try {
      ...
      await fetchEvents();
      onSave?.();
      toast({ title: "반복 일정이 수정되었습니다.", status: "success", duration: 3000, isClosable: true });
    } catch {
  ...
  const deleteRepeatEvents = async (eventIds: string[]) => {
    try {
      ...
      await fetchEvents();
      toast({ title: "반복 일정이 삭제되었습니다.", status: "info", duration: 3000, isClosable: true });
    } catch {
  ...
  return { events, fetchEvents, saveEvent, deleteEvent, createRepeatEvent, updateRepeatEvents, deleteRepeatEvents };
};
