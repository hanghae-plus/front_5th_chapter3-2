...
test.describe.serial('반복 일정 수정/삭제 테스트', () => {
  ...
  test('반복 일정 중 하나를 수정/삭제해도 나머지는 유지되어야 한다', async ({ page }) => {
    ...
    await expect(page.getByTestId('event-title').filter({ hasText: /^수정 테스트$/ })).toHaveCount(
      1
    );
    await expect(page.getByTestId('event-title').filter({ hasText: /^반복 테스트$/ })).toHaveCount(
      2
    );
