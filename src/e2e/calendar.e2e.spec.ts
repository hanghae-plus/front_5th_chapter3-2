...
test.describe('캘린더 E2E', () => {
  ...
  test('반복 일정을 생성하고 수정하면 단일 일정으로 변경되고 그것을 삭제하면 단일로 삭제 된다.', async ({
    ...
    const checkBox = page.getByText('반복 일정');
  ...
  test('단일 일정을 여러 개 생성하고 수정시 원하는 일정만 수정된다.', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: '제목' }).fill(events[0].title);
    await page.getByRole('textbox', { name: '날짜' }).fill(events[0].date);
    await page.getByRole('textbox', { name: '시작 시간' }).click();
    await page.getByRole('textbox', { name: '시작 시간' }).fill(events[0].startTime);
    await page.getByRole('textbox', { name: '종료 시간' }).click();
    await page.getByRole('textbox', { name: '종료 시간' }).fill(events[0].endTime);
    await page.getByRole('button', { name: '일정 추가' }).click();

    await page.getByRole('textbox', { name: '제목' }).fill(events[1].title);
    await page.getByRole('textbox', { name: '날짜' }).fill(events[1].date);
    await page.getByRole('textbox', { name: '시작 시간' }).click();
    await page.getByRole('textbox', { name: '시작 시간' }).fill(events[1].startTime);
    await page.getByRole('textbox', { name: '종료 시간' }).click();
    await page.getByRole('textbox', { name: '종료 시간' }).fill(events[1].endTime);
    await page.getByRole('button', { name: '일정 추가' }).click();

    await page.getByRole('textbox', { name: '제목' }).fill(events[2].title);
    await page.getByRole('textbox', { name: '날짜' }).fill(events[2].date);
    await page.getByRole('textbox', { name: '시작 시간' }).click();
    await page.getByRole('textbox', { name: '시작 시간' }).fill(events[2].startTime);
    await page.getByRole('textbox', { name: '종료 시간' }).click();
    await page.getByRole('textbox', { name: '종료 시간' }).fill(events[2].endTime);
    await page.getByRole('button', { name: '일정 추가' }).click();
